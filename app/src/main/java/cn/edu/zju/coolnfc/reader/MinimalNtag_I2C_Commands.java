...
import cn.edu.zju.coolnfc.exceptions.CC_differException;
import cn.edu.zju.coolnfc.exceptions.CommandNotSupportedException;
import cn.edu.zju.coolnfc.exceptions.DynamicLockBitsException;
import cn.edu.zju.coolnfc.exceptions.NotPlusTagException;
import cn.edu.zju.coolnfc.exceptions.StaticLockBitsException;
...
public class MinimalNtag_I2C_Commands extends I2C_Enabled_Commands {
	...
	public byte[] getSessionRegisters() throws IOException, FormatException,
		...
		if (tagType == Ntag_Get_Version.Prod.NTAG_I2C_1k || tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k) {
	...
	public byte[] getConfigRegisters() throws IOException, FormatException,
			CommandNotSupportedException {
		if (tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k) {
			throw new CommandNotSupportedException(
	...
	public void writeEEPROM(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException,
			CommandNotSupportedException {
		if ((tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k || tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k_Plus)
	...
	public byte[] readEEPROM(int absStart, int absEnd) throws IOException,
		...
		if ((tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k && absEnd > 0xFF)
				|| tagType == Ntag_Get_Version.Prod.NTAG_I2C_2k_Plus && absEnd > 0xE1)
			throw new CommandNotSupportedException(
	...
	public int getProtectionPlus() {
		try {		
			...
			if(auth0 != null && auth0.length < 4) {
				...
				return AuthActivity.AuthStatus.Protected_RW_SRAM.getValue();
			} else {
					...
					if (((0x0000080 & access[0]) >> Access_Offset.NFC_PROT.getValue() == 1)
					 && ((0x0000004 & pti2c[0]) >> PT_I2C_Offset.SRAM_PROT.getValue() == 1)) {
						...
						return AuthActivity.AuthStatus.Protected_RW.getValue();
						  ...
						  && ((0x0000004 & pti2c[0]) >> PT_I2C_Offset.SRAM_PROT.getValue() == 1)) {
						return AuthActivity.AuthStatus.Protected_W_SRAM.getValue();
					} else if (((0x0000080 & access[0]) >> Access_Offset.NFC_PROT.getValue() == 0)
						  && ((0x0000004 & pti2c[0]) >> PT_I2C_Offset.SRAM_PROT.getValue() == 0)) {
						return AuthActivity.AuthStatus.Protected_W.getValue();
					}
				}
			}
			return AuthActivity.AuthStatus.Unprotected.getValue();
		} catch (IOException e) {
			e.printStackTrace();
		} catch (FormatException e) {
			e.printStackTrace();
