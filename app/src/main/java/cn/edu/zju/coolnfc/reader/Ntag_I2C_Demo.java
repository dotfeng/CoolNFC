package cn.edu.zju.coolnfc.reader;
...
import cn.edu.zju.coolnfc.activities.AuthActivity;
import cn.edu.zju.coolnfc.activities.FlashMemoryActivity;
...
import cn.edu.zju.coolnfc.activities.VersionInfoActivity;
import cn.edu.zju.coolnfc.crypto.CRC32Calculator;
import cn.edu.zju.coolnfc.exceptions.CommandNotSupportedException;
import cn.edu.zju.coolnfc.exceptions.NotPlusTagException;
...
public class Ntag_I2C_Demo implements WriteEEPROMListener, WriteSRAMListener {
			...
			if (!prod.equals(Ntag_Get_Version.Prod.Unknown)) {
				if (prod.equals(Ntag_Get_Version.Prod.NTAG_I2C_1k_Plus)
			     || prod.equals(Ntag_Get_Version.Prod.NTAG_I2C_2k_Plus)) {
					...
					if(authStatus == AuthActivity.AuthStatus.Authenticated.getValue()) {
	...
	public void setBoardVersion() throws IOException, FormatException,
		...
		if (!((reader.getSessionRegister(I2C_Enabled_Commands.SR_Offset.NC_REG) & I2C_Enabled_Commands.NC_Reg_Func.PTHRU_ON_OFF
				.getValue()) == I2C_Enabled_Commands.NC_Reg_Func.PTHRU_ON_OFF.getValue())) {
		...
		for (int i = 0; i < TRAILS; i++) {
			if (((reader.getSessionRegister(I2C_Enabled_Commands.SR_Offset.NS_REG) & I2C_Enabled_Commands.NS_Reg_Func.SRAM_RF_READY
					.getValue()) == I2C_Enabled_Commands.NS_Reg_Func.SRAM_RF_READY.getValue())) {
				break;
			}
			try {
				Thread.sleep(1);
	...
	public Boolean Flash(byte[] bytesToFlash) {
		...
		try {
			...
			for (int i = 0; i < flashes; i++) {
				...
				reader.writeSRAM(flashData, I2C_Enabled_Commands.R_W_Methods.Fast_Mode, this);
	...
	private class SRAMSpeedtestTask extends AsyncTask<Void, String, Void> {
		...
		protected void onPreExecute() {
			...
			if (SpeedTestFragment.getReadOptions().equalsIgnoreCase(
				...
				method = I2C_Enabled_Commands.R_W_Methods.Polling_Mode;
				...
				method = I2C_Enabled_Commands.R_W_Methods.Error;
			}
		}
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				if (prod.equals(Ntag_Get_Version.Prod.NTAG_I2C_1k_Plus)
				 || prod.equals(Ntag_Get_Version.Prod.NTAG_I2C_2k_Plus)) {
					if(MainActivity.getAuthStatus() == AuthActivity.AuthStatus.Authenticated.getValue()) {
				...
				for (int i = 0; i < Blocks; i++) {
					...
					publishProgress(((i + 1) * reader.SRAMSize)
							+ " Bytes written");
					if (method == I2C_Enabled_Commands.R_W_Methods.Polling_Mode) {
						reader.waitforI2Cread(DELAY_TIME);
					} else {
						try {
							// else wait
