...
import cn.edu.zju.coolnfc.adapters.TabsAdapter;
import cn.edu.zju.coolnfc.fragments.ConfigFragment;
...
public class MainActivity extends FragmentActivity {
	...
	public static void launchNdefDemo(int auth, byte[] pwd) {
		if (demo.isReady()) {
			if (demo.isConnected()) {
				if(auth == AuthActivity.AuthStatus.Authenticated.getValue()) {
					demo.Auth(pwd, AuthActivity.AuthStatus.Protected_RW.getValue());
				}
				NdefFragment.setAnswer("Tag detected");
				try {
					demo.NDEF();
	...
	public void updateAuthIcon(int status) {
		if(mAuthMenuItem != null) {
			if(status == AuthActivity.AuthStatus.Disabled.getValue()) {
				mAuthMenuItem.setIcon(getResources().getDrawable(R.mipmap.disabled));
			} else if(status == AuthActivity.AuthStatus.Unprotected.getValue()) {
				mAuthMenuItem.setIcon(getResources().getDrawable(R.mipmap.unlock));
			} else if(status == AuthActivity.AuthStatus.Authenticated.getValue()) {
				mAuthMenuItem.setIcon(getResources().getDrawable(R.mipmap.authenticated));
			} else if(status == AuthActivity.AuthStatus.Protected_RW.getValue()
					|| status == AuthActivity.AuthStatus.Protected_W.getValue()
					|| status == AuthActivity.AuthStatus.Protected_RW_SRAM.getValue()
					|| status == AuthActivity.AuthStatus.Protected_W_SRAM.getValue()) {
